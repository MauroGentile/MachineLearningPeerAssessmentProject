...
After an initial preprocessing, data will be split into a training set, representing the 70% of available data, and a validation set representing the remaining 30% of data. A total of 7 different classification models (Random Forest, Knn, C5, GBM, Linear, Quadratic and Flexible Discrmininant Analysis) will be then built on the training set and optimized through a 10 folds cross validation.
...
As a result, it will be shown that Random Forest and C5 are the best performing alghorithms, both reaching a 99,4% accuracy on the validation set or, in other words, an Out of Sample Error of 0,6% indicating a really good fit of both models.

However C5 algorithm takes 1/4 of elapse time to train the same data set: on my iMac 2.7 GHz Core i5 with 8 GB 1600 DDR3 memory, while RF took 1 hr and 30 minutes to make a 10 cross validation on the training set, C5 took only 23 minutes to perform the same operation on the same data set.  

In other words, RF and C5 are indistinguishable models from an accuracy point of view but C5 is a much faster algorithm than Random Forest. At least on this data set. For this reason, C5 is chosen as best model.  

Nevertheless, if computing resources or timing is an issue and the training procedure has to be frequently repeated (for instance if the data set gets frequently updated), Knn may become an excellent alternative because while not reaching an accuracy as high as C5, it's still an excellent model with a superb 96,6% accuracy but taking only 1 minute and a half to train the data, 60 times lower than Random Forest and 15 time lower than C5.

In other words, in special circumstances, we may end up preferring Knn over C5, foregoing a little bit of accuracy for the sake of training speed.
...
While the graphic comparison of accuracy and confidence intervals of the 7 models will be performed in the next paragraph, the tabular version of all these confusion matrices can be found the Appendix 1.  


### Plot of accuracies on validation set (which is equivalent to 1-Out of Sample Error)

In the following plot, accuracy on the validation set of each method will be displayed together with its confidence interval. The corresponding out of sample error is the complement to 1 of accuracy.

Two methods stands out from the rest and are virtually indistinguishable one other: Random Forest and C5 with an accuracy of 99,4% and a very narrow confidence interval. The corresponding Out of Sample Error is 0.6%.

Other 2 well performing techniques are the Knn and GBM with an accuracy of 96,6% and 95,8% respectively but a wider confidence interval with respect to RF and C5.

The discriminant analysis models performance are much worse with accuracies lower than 90%. The LDA is especially poor with an accuracy as low as 71,1%.
...
As you can see, Random Forest is an extremely resource consuming algorithm. It took 5.596 sec to train the data set on my iMac 2.7 GHz Core i5 with 8 GB 1600 DDR3 memory.

The C5 algorithm, while having the same exact accuracy, takes 1/4 of time to train the same training data (1.387 sec).

C5 is therefore a better model than Random Forest at least on this data set since while reaching the same accuracy, it is also much faster.

Stretching this concept to an extreme, depending on how often the user is called to update the model building phase on (for instance on data set which may be updated frequently), one may also consider to use the Knn algorithm instead of C5 and Random Forest, because while not reaching the same accuracy (96,6% for the Knn and 99,4% for the C5 and Random Forest), Knn took only 1 minute and a half to train the data set while Random Forest took 1hr and 30 minutes and C5 approximately 23 minutes to perform the same task on the same dataset my computer.
...
ggplot(Models,aes(x=accuracy, y=elapsed_time)) + geom_point() + geom_text(aes(x=accuracy, label=name), size=4, vjust=-1.3)+ xlim(0.6, 1.1)+ ylim(0, 6300)
...
As last step in this project, I would like to compare performance of Random Forest, C5 and Knn models, i.e the top 3 performing algorithms, on the 20 row data set provided in the project.
Comparison will be performed both in terms of accuracy and in terms of testing elapsed time. Please notice that in this case, the elapsed time being considered is NOT the time to build the model (as above) but the time needed to make 20 predictions using already available models (built above).
...
Let's count the output difference between RF/C5 and between C5/Knn.  
...
Knn was `r c5_knn_ratio` faster than C5 in making predictions.

In contrast with the expectations, Knn produced 1 prediction different than the other 2 models.  
On the other hand, as expected, Knn was the fastest model to perform the predictions.

## CONCLUSIONS

We have seen that while Random Forest and C5 are the best model on this data set and virtually indistinguishable each other with an Out of Sample error of 0,6%, C5 is much better than Random Forest since it reaches the same results in a much shorter time. C5 is therefore chosen as preferred model on this data set.
